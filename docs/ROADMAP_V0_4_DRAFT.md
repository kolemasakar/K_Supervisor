# ROADMAP v0.4 — DRAFT
Proposed successor roadmap after completed ROADMAP v0.3.
...
The v0.4 candidate should make the existing control plane practically operable through supported API/CLI surfaces, add a governed production model-inference adapter, one real remote repository provider, current Plugin-native release assets, and stronger operational telemetry/supply-chain evidence.
...
This document is a proposal only. No v0.4 phase is active until the owner explicitly approves ROADMAP v0.4 (or an amended revision). After approval, canonical `ROADMAP.md`, `TEST_MATRIX.md`, `PROJECT_STATE.md` and supporting baseline documents must be synchronized through the protected `main` pull-request gate before Phase 0 completion or any runtime implementation.
